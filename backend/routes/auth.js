...
const verifyStateToken = (stateToken) => {
  ...
  // Signature must be a valid SHA-256 hex digest (64 characters)
...
async function findOrCreateGoogleUser(payload) {
  ...
  // 2. Check if user already exists with matching verified email
  // Account Linking
...
router.get('/google/callback', async (req, res) => {
  ...
  if (error) {
    console.warn(
      'Google sign-in cancelled or failed by user:',
      error
    );
  ...
  if (!code) {
    return res.redirect(
      `${frontendUrl}/login?error=${encodeURIComponent(
        'Authorization code missing from Google.'
      )}`
    );
  }
  ...
  if (!verifyStateToken(state)) {
    console.warn(
      'Invalid or expired OAuth state parameter received'
    );
  ...
  try {
    ...
    res.redirect(
      `${frontendUrl}/login?error=${encodeURIComponent(userMsg)}`
    );
  }
});
...
async function getFirebasePublicCerts() {
  ...
  if (!res.ok) {
    throw new Error(
      'Failed to retrieve Firebase public signing certificates.'
    );
  }
  ...
  const ttlSeconds = maxAgeMatch
    ? parseInt(maxAgeMatch[1], 10)
    : 6 * 3600;

  firebaseCertsExpiresAt =
    now + ttlSeconds * 1000;

  return certs;
}

// Direct ID Token verification endpoint
// Supports both Firebase Auth ID Tokens and Google OAuth ID Tokens
  ...
  try {
    ...
    if (!credential) {
      return res.status(400).json({
        message:
          'Google/Firebase ID token credential is required.',
      });
    }
    ...
    const projectId =
      process.env.FIREBASE_PROJECT_ID || 'bsafe-f874c';
    ...
    if (isFirebaseToken) {
      ...
      if (!kid) {
        return res.status(401).json({
          message:
            'Missing key ID in Firebase token header.',
        });
      }
      ...
      if (!cert) {
        return res.status(401).json({
          message:
            'Invalid or unknown signing key for Firebase token.',
        });
      }
      ...
      if (!process.env.GOOGLE_CLIENT_ID) {
        return res.status(500).json({
          message:
            'Google OAuth client ID is not configured on the server.',
        });
      }
...
router.post('/register', async (req, res) => {
  try {
    ...
    // Users can register as normal users or volunteers.
    // Admin creation must remain server-controlled.
    const userRole = ['user', 'volunteer'].includes(role)
      ? role
      : 'user';
  ...
  async (req, res) => {
    try {
      ...
      if (location) {
        ...
        // The frontend must send actual [longitude, latitude].
        if (
